...
import { buildGraph, type LaneKey, type MapGraph } from "@/lib/map-data";
...
/**
 * Filter graph by lane keys, host, and orphans-only switch.
...
export function applyFilters(graph: MapGraph, filters: MapFilters): MapGraph {
  ...
  const keepNode = (id: string): boolean => {
    ...
    // Lane filter — host always passes (no lane); agents/ds must match
    if (!allLanes) {
