...
export async function apiResponse(action: (requestId: string) => Promise<Response>): Promise<Response> {
  ...
  try { response = await action(requestId); }
    ...
    if (error instanceof ZodError) {
      response = Response.json({ error: { code: "invalid_document", message: "Validation failed.", requestId,
      ...
      response = Response.json({ error: { code: failure.code, message: failure.message, requestId } }, { status: failure.status });
    }
  }
